...
select userid,count(created_date) total_visits from sales group by userid order by count(created_date) desc
...
(select userid,product_id,count(product_id) cnt from sales group by userid,product_id) as a)b
where rnk=1
...
(select a.userid,a.created_date,a.product_id,b.gold_signup_date from sales a inner join goldusers_signup b on a.userid=b.userid and created_date<=gold_signup_date) c)d where rnk=1
--8 what is the total order and amount spent for each member before they become a gold member?
...
--9 if buying each product generates points for eg 5rs=2 tomato point and each product has different purchasing points for eg for p1 5rs=1 tomato point ,for p2 10rs=5 tomato points and p3 5rs=1 tomato points,
--calculate points collected by each customer and for which product most points have been given till now?
select userid,sum(total_points)*2.5 total_money_earned from 
(select e.*,amt/points  total_points from (select d.*,case when product_id=1 then 5 when product_id=2 then 2 when product_id=3 then 5 else 0 end as points from 
...
--what the customer has purchased they earn 5 zomato points for every 10 rs spent who earned more 1 or 3 and what was their points earnings in their first yr?
-- 1zp=2rs
--o.5 zp=1 rs
...
--12 rank all the transactions for each  member whenever they are a zomato gold member for every non gold member transaction maark as na
select e.*, case when rnk=0 then 'na' else rnk end as rank from (select c.*, cast((case when gold_signup_date is null then 0 else rank() over(partition by userid order by created_date desc) end) as varchar) as rnk from 
